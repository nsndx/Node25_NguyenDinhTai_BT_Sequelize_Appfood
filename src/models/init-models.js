...
function initModels(sequelize) {
  ...
  restaurant.belongsToMany(user, { as: 'list_users_like', through: like_res, foreignKey: "res_id", otherKey: "user_id" });
  restaurant.belongsToMany(user, { as: 'list_users_rate', through: rate_res, foreignKey: "res_id", otherKey: "user_id" });
  user.belongsToMany(food, { as: 'food_id_foods', through: order, foreignKey: "user_id", otherKey: "food_id" });
  user.belongsToMany(restaurant, { as: 'list_restaurants_like', through: like_res, foreignKey: "user_id", otherKey: "res_id" });
  user.belongsToMany(restaurant, { as: 'list_restaurants_rate', through: rate_res, foreignKey: "user_id", otherKey: "res_id" });
